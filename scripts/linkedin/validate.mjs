...
export function validate(draft) {
  ...
  // CTA discipline
  const ctas = (body.match(/\b(reply|message me|get in touch|book a call|reach out|happy to share|send me)\b/gi) || []).length;
  if (ctas > 1) errors.push(`${ctas} calls to action (max 1)`);
  ...
  return { ok: errors.length === 0, errors, warnings: [...new Set(warnings)], chars, hookChars: hook.length };
}
