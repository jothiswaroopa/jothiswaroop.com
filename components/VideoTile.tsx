    ...
    <div className={`relative bg-ink ${vertical ? "aspect-[9/16]" : "aspect-video"} ${className}`}>
      {play ? (
        ...
        <button onClick={() => setPlay(true)} className="press group absolute inset-0 grid place-items-center" aria-label={`Play: ${title}`}>
          <img ref={imgRef} src={poster} alt="" onLoad={onPoster} onError={onPoster} className="absolute inset-0 h-full w-full object-cover opacity-85 transition-opacity duration-300 group-hover:opacity-100" />
