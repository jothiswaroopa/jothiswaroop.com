              ...
              <Reveal key={i} delay={i * 0.06}>
                <div className="border-b hairline">
                  <button onClick={() => setOpen(isOpen ? null : i)} className="flex w-full items-center justify-between gap-6 py-6 text-left !select-text" aria-expanded={isOpen}>
                    ...
                    <span className={`mono text-2xl text-paper/70 transition-transform duration-300 ease-out-expo ${isOpen ? "rotate-45" : ""}`}>+</span>
                  </button>
                  <AnimatePresence initial={false}>
                    {isOpen && (
                      <m.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.35, ease: EASE }} className="overflow-hidden">
                        <p className="max-w-2xl pb-8 text-paper/80">{f.a}</p>
                      </m.div>
                    )}
                  </AnimatePresence>
                </div>
