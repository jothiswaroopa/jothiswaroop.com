            ...
            <ul className="mt-6 space-y-5">
              {filter.forList.map((l, i) => (
                <Reveal key={i} delay={i * 0.08} y={16}>
                  <li className="flex gap-4 border-b hairline pb-5 text-lg text-paper/90"><span className="mono text-xs text-paper/60 mt-1.5">✓</span>{l}</li>
                </Reveal>
              ))}
            </ul>
          </div>
            ...
            <ul className="mt-6 space-y-5">
              {filter.notList.map((l, i) => (
                <Reveal key={i} delay={0.1 + i * 0.08} y={16}>
                  <li className="flex gap-4 border-b hairline pb-5 text-lg text-paper/70"><span className="mono text-xs text-strike mt-1.5">×</span>{l}</li>
                </Reveal>
              ))}
            </ul>
          </div>
