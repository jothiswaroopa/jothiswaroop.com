  ...
  return (
    <section id="notes" className="bg-ink">
      <div className="mx-auto max-w-[1440px] px-5 py-20 md:px-10 md:py-28">
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-3">
            {notes.posts.map((p, i) => (
              <Reveal key={p.title} delay={i * 0.08}>
                <a href={p.href} className="group block h-full rounded-2xl border hairline bg-ink-2 p-6 transition-colors hover:border-line-strong">
        ...
        <Reveal delay={0.3} className="mt-12 flex flex-col gap-4 rounded-2xl border hairline bg-ink-2 p-6 md:flex-row md:items-center md:justify-between md:p-8">
          ...
          <form className="flex w-full gap-2 md:w-auto" action="/audit">
            ...
            <button className="shrink-0 rounded-full bg-signal px-5 py-3 text-sm font-medium text-ink hover:bg-paper">Join</button>
          </form>
        </Reveal>
      </div>
    </section>
